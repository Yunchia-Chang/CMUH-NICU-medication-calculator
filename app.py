...
drug_data = {
    ...
    "Amoxicillin 1.0g + Clavulanate 0.2g (1.2g/vial)": ["1vial加入20mL N/S (1mL=60mg A+C)，取實際dose，以N/S稀釋2.5倍 (1ml=24mg A+C),建議用1.5mL N/S drip 30 mins", "20mL N/S", "-", "dose/60", "N/S 2.5倍", "-", "-", "F*2.5", "30"],
    "Ampicillin/Sulbactam (A+S 1.5g/vial)": ["1 vail 加入 3.2 mL N/S (1mL=375mg ampicillin+sulbactam)，取實際dose，稀釋成10倍量 (1mL=37.5mg)，建議用1.5mL N/S drip 30 mins.", "3.2mL N/S", "-", "dose/375", "N/S 10倍", "-", "-", "F*10", "30"],
    "Piperacillin + Tazobactam (2.25g/vial)": ["1 vial加入10 mL N/S (1mL=202.5mg PIP+TAZO), 取實際dose, 稀釋成2.5倍量 (1mL=81 mg PIP+TAZO)，建議用1.5mL N/S drip 30 mins", "10mL N/S", "-", "dose/202.5", "N/S 2.5倍", "-", "-", "F*2.5", "30"],
    "Ertapenem 1g/Vial": ["1 vail 加入 10mL 注射用水配置 (1mL=100mg)，取實際dose，以N/S稀釋成5倍量 (1mL=20mg) 給藥，建議用1.5mL N/S drip 30 mins.", "10mL注射用水", "-", "dose/100", "N/S 5倍", "-", "-", "F*5", "30"],
    ...
    "Colistin (2MU/Vial=66.8mg CBA)": ["1vial加入2mL N/S (1mL=1MU=33.4mg CBA)，取實際dose，稀釋成2倍量(1mL=16.7mg)，不足1mL者加到1mL，建議用1.5mL N/S drip 30 mins", "2mL N/S", "-", "dose/33.4", "N/S 2倍", "-", "-", "F*2", "30"],
    "Clindamycin 300mg/2mL/Amp": ["取實際dose，稀釋成25倍量 (1mL=6mg)給藥，建議用1.5mL N/S drip 30 mins", "-", "dose/150", "-", "N/S 25倍", "-", "-", "D*25", "30"],
    ...
    "Bumetanide (2mg/4mL/Amp)": ["取實際dose, 稀釋為2倍量 (1ml=0.25mg), IVP 1-2 mins", "-", "dose/0.5", "-", "D5W N/S 2倍", "-", "-", "D*2", "1-2"],
    "Ibuprofen (10mg/2mL/Amp)": ["不稀釋取實際dose，建議用1.5mL N/S drip 30 mins", "-", "dose/5", "-", "-", "-", "-", "D", "30"],
    "Propacetamol 1 g/Vial": ["1 vial 加入 5mL 溶解液 (1mL=200mg) 配置，取實際dose，以N/S稀釋成10倍量 (1ml=20mg) ，建議用1.5 mL N/S drip 15 mins", "5mL 溶解液", "-", "dose/200", "N/S 10倍", "-", "-", "F*10", "15"],
    ...
    "Hydrocortisone (100mg/Vial) IVP": ["1vial加入2mL N/S (1mL=50mg) 配置， 取實際dose, IVP>30 sec", "2mL N/S", "-", "dose/50", "-", "-", "-", "F", ">30sec"],
    "Famotidine (20mg/2mL/Amp)": "SPECIAL_FAMO",
    "Dexamethasone (5mg/mL/Amp)": "SPECIAL_DEX",
    "Hydrocortisone (100mg/Vial) IVD": "SPECIAL_HYDRO",
    "Methylprednisolone (40 mg/Vial)": "SPECIAL_METHYL"
}
...
if selected_name != "-- 請選擇 --":
    ...
    if drug_data[selected_name] == "SPECIAL_HYDRO":
        ...
        if dose > 0:
            if (dose / 50.0) <= 0.1:
                res.update({
                    "F": "抽0.1mL, dilute to 1mL", "G": "-", "I": "NS 5倍", "F2": f"{dose/5.0:.3f}", "J": f"{(dose/5.0)*5:.3f}"
                })
                show_warning = True
            else:
                res["nicu"] = "1vial加入2mL N/S (1mL=50mg) 配置，取實際dose，再稀釋50倍(1mL=1mg)，建議用1.5mL N/S drip 30 mins"
                res.update({
        ...
        if dose > 0:
            if (dose / 10.0) <= 0.1:
                res.update({
                    "F": "抽0.1mL, dilute to 1mL", "G": "-", "I": "NS 5倍", "F2": f"{dose:.3f}", "J": f"{dose*5:.3f}"
                })
                show_warning = True
            else:
                res["nicu"] = "抽實際dose(1mL=10mg)，再稀釋50倍量 (1mL=0.2mg)，建議用1.5mL N/S drip 30 mins"
                res.update({
                    "D": f"{dose/10.0:.3f}", "G": "NS 50倍", "I": "-", "F2": "-", "J": f"{(dose/10.0)*50:.3f}"
                })
        
    elif drug_data[selected_name] == "SPECIAL_DEX":
        res["E"] = "-"
        res["time"] = "30"
        res["nicu"] = "抽0.1mL (1mL=5mg)，稀釋成1mL (1mL=0.5mg)，抽實際dose，建議用1.5mL N/S drip 30 mins"
        if dose > 0:
            if (dose / 5.0) <= 0.1:
                res.update({
                    "D": "抽0.1mL, dilute to 1mL", "G": "-", "I": "-", "F2": f"{(dose/0.5):.3f}", "J": f"{(dose/0.5):.3f}"
                })
                show_warning = True
            else:
                res["nicu"] = "不需稀釋(=5mg/mL) 取實際dose，建議用1.5mL N/S drip 30 mins"
                res.update({
                    "D": f"{dose/5.0:.3f}", "G": "-", "I": "-", "F2": "-", "J": f"{dose/5.0:.3f}"
                })

    elif drug_data[selected_name] == "SPECIAL_METHYL":
        res["E"] = "1mL 注射用水"
        res["time"] = "30"
        res["nicu"] = "1vial加入1mL 注射用水 (1mL=40mg)，抽0.1mL (1mL=40mg)，稀釋成1mL (1mL=4mg)，抽實際dose，建議用1.5mL N/S drip 30 mins"
        if dose > 0:
            if (dose / 40.0) <= 0.1:
                res.update({
                    "F": "抽0.1mL, dilute to 1mL", "G": "-", "I": "-", "F2": f"{(dose/4.0):.3f}", "J": f"{(dose/4.0):.3f}" 
                })
                show_warning = True
            else:
                res["nicu"] = "1vial加入1mL 注射用水 (1mL=40mg)，取實際dose，稀釋成8倍量 (1mL=5mg)，建議用1.5mL N/S drip 30 mins."
                res.update({
                    "F": f"{dose/40.0:.3f}", "G": "N/S 8倍", "I": "-", "F2": "-", "J": f"{(dose/40.0)*8:.3f}"
                })
